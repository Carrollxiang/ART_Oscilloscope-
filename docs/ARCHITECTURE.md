...
## 7. v0.4 重构决议 (反馈优先 + 事件窗口测量)

> 本节用于落地当前版本的系统优先级调整。若与旧章节描述冲突, 以本节为准。

### 7.1 系统优先级

按运行时优先级重新排序:

1. **反馈闭环 (最高优先级)**: 采集后第一时间提取反馈所需特征并发送。
2. **交互流畅度 (高优先级)**: 保存配置、修改测量参数、编辑反馈项不被采集/绘图阻塞。
3. **主示波器显示 (中优先级)**: 允许丢帧, 但保持实时感知。
4. **Mini Chart (低优先级)**: 随硬件触发同步更新, 允许抽稀、丢旧帧, 不得影响前 3 项。

### 7.2 从通道级 Pipeline 到事件窗口级 Pipeline

旧模式主要按整通道计算 (`CHx_Vpp`, `CHx_Freq`)。该模式不适合同一通道时空复用不同物理量的场景。
...
class EventWindowSpec:
    tag: str                   # 语义名, 例如 "A_power" / "B_power"
    channel: str               # "CH1"...
    start_ms: float
    end_ms: float
    feature: str               # "Vpp" / "Vrms" / "Integral"...
    semantic: str = ""         # 可选说明
```

设计约束:

- 同一通道可配置多个窗口, 每个窗口独立计算、独立命名、独立订阅。
- 反馈系统优先订阅 `tag` 对应的窗口值, 不依赖整通道全局统计。
- 通道级测量保留给监控与诊断, 但不作为反馈主数据源。

### 7.3 数据面/控制面分离

为保证反馈实时性与 UI 流畅性, 运行时拆分为两条路径:

- **数据面**: `Acquisition -> EventWindowProcess -> FeedbackDispatch -> UIRender`
- **控制面**: `UI操作(保存/修改参数/启停slot) -> ControlQueue -> 原子应用配置`

原则:

- 数据面不直接执行 UI 控件修改。
- 控制面不与数据面共享可变对象, 使用快照或原子替换。
- 配置切换在“帧边界”应用, 避免半帧状态不一致。

### 7.4 队列与背压 (Backpressure)

**定义**: 当下游处理速度低于上游产出速度时, 用有界队列与丢弃/覆盖策略阻止延迟无限增长。

推荐队列策略:

| 队列 | 用途 | maxsize | 满队列策略 | 说明 |
|------|------|---------|------------|------|
| `FeedbackQueue` | 反馈闭环数据 | 1~2 | `drop_oldest` | 永远优先最新帧 |
| `UIQueue` | 主示波器渲染 | 1 | `drop_oldest` | UI 允许丢帧 |
| `MiniChartQueue` | 趋势图渲染 | 1 | `drop_oldest` | 触发驱动, UI 忙时丢旧保新 |
| `ControlQueue` | 参数/保存/启停命令 | 8 (可调) | `block` 或显式拒绝 | 不能静默丢控制命令 |

补充约束:

- 禁止无界任务堆积 (`run_coroutine_threadsafe` 需配合有界队列/闸门)。
- 反馈发送使用固定大小执行池, 不允许按帧无限增线程。
- 所有队列必须上报指标: `qsize`, `drop_count`, `latency_ms`。

### 7.5 Mini Chart 触发驱动渲染策略

Mini Chart 仅用于趋势监控, 采用以下策略:

- 每次硬件触发最多更新一次 (与测量更新同节拍)。
- `mock` 模式与硬件模式保持同一规则: 采集产出一帧, Mini Chart 更新一帧。
- Mini Chart 不允许自建独立刷新节拍 (`QTimer`)；允许保留“脏标记+一次性刷新”的实现细节，但触发源必须来自采集帧事件。
- 每次最多绘制最近 `N` 点 (建议 `300~1000`)。
- 数据更新与曲线对象复用 (`setData`), 禁止频繁重建图元。
- 任何异常仅记录日志并限流通知, 不得阻塞主 UI。
