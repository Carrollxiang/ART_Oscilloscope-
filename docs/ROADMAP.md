...
| 限制 | 说明 | 计划 |
|------|------|------|
| 触发源固定 | 默认 ai12, 1V, 上升沿 | ✅ 已实现 UI 配置 (DevicePanel 硬件触发组) |
...
| 方向 | 说明 | 优先级 |
|------|------|--------|
| ~~触发源 UI 配置~~ | ✅ 已实现: DevicePanel 硬件触发组 (源/电平/斜率), 默认值仍为 ai12/1V/上升沿 | ✅ |
| 更多测量特征 | Freq, Period, DutyCycle (需过零检测算法) | 🟡 中 |
| ~~单点/连续模式切换~~ | ✅ 已实现: acquisition_mode (v0.8), 默认 CONTINUOUS | ✅ |
