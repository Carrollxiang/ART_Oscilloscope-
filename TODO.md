...
> 2026/8: 内存泄漏修复后 mock 长跑 2 小时私有内存趋平 (修复前 ~500MB/h, 修复后趋近 0),
>         根因为 pyqtgraph useOpenGL(True) 每帧 setData 的 GL 缓存未释放 (v0.8.2);
> 以下 UI 交互项仍待系统化验证。

- [x] 长时间运行反馈队列 qsize 不持续堆积 (6h 真机隐含验证)
- [x] 反馈延迟不随时间增长 (6h 真机隐含验证)
- [x] 长时间运行内存不持续增长 (mock 长跑验证, 修复前 ~500MB/h → 修复后趋平, v0.8.2)
- [ ] 修改测量类型/时间参数流畅
- [ ] 保存配置无明显卡顿
- [ ] 同一订阅项在测量面板与反馈面板读数一致
